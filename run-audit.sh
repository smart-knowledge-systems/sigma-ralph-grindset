...
#
# Usage: ./run-audit.sh [--all] [--max-loc N] <policy-name> [policy-name2 ...]
#        ./run-audit.sh -h|--help
# ============================================================================
...
# Argument parsing: optional --all flag, --max-loc flag + required policy name(s)
FORCE_ALL=""
POLICY_NAMES=()
...
process_branch() {
    ...
    while read -r issue; do
        ...
        local file_path file_id
        while read -r file_path; do
            file_path=$(sql_escape "$file_path")
            db "INSERT OR IGNORE INTO files (path) VALUES ('$file_path');"
            file_id=$(db "SELECT id FROM files WHERE path='$file_path';")
            db "INSERT INTO issue_files (issue_id, file_id) VALUES ($issue_id, $file_id);"
        done < <(printf '%s\n' "$issue" | jq -r '.files[]')
